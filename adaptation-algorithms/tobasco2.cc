            ...
            if ((m_lastRepIndex != 0) 
              && ((8.0 * m_videoData.segmentSize.at(m_lastRepIndex).at(segmentCounter - 1)) / timeFactor >= lastSegmentThroughput))
              {
                ...
                for (int i = m_highestRepIndex; i >= 0; i--)
                  {
                    if ((8.0 * m_videoData.segmentSize.at (i).at (segmentCounter - 1)) / timeFactor >= lastSegmentThroughput) continue;
                    else 
                      {
                        nextRepIndex = i;
                        break;
...
TobascoAlgorithm::MinimumBufferLevelObserved ()
{
  ...
  if (m_deltaBeta < m_videoData.segmentDuration)
    {
      if (lastPackage - secondToLastPackage < m_deltaBeta) return true;
      else return false;
    }
  else 
    {
      if (lastPackage - secondToLastPackage < m_videoData.segmentDuration) return true;
      else return false;
    }
}
...
{
  if (t_1 < 0)  t_1 = 0;
  if (m_throughput.transmissionEnd.size() < 1)  return 0;
  ...
  for (uint i = 0  ; i <= m_throughput.transmissionEnd.size () - 1; i++)
    {
      if (m_throughput.transmissionEnd.at (i) < t_1) continue;
      else 
        {
          index = i;
          break;
